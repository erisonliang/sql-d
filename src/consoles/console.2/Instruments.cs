...
namespace SqlD.Console_2
{
	public static class Instruments
	{
		...
		public static void Start()
		{
			...
			{
				Log.Out.Info("Starting performance reporter for READS (RT = Realtime, AV = Accumulative Value)");
				while (!totalPrintTaskCancelSource.Token.IsCancellationRequested)
				{
					Log.Out.InfoInline($"RT: {previousTotalRealtimeRecords}/sec AV: {Math.Round(totalRecords / totalSeconds, 0)}/sec");
					Thread.Sleep(TimeSpan.FromSeconds(1));
				}
			});
		}
